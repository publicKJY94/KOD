...
public class AddressDAO {
	...
	private static final String SELECTALL = "SELECT "
			+ "ADDRESS_ID, ADDRESS_NAME, ADDRESS_STREET, ADDRESS_LOTNUM, ADDRESS_DETAIL, ADDRESS_ZIPCODE "
			+ "FROM ADDRESS "
			+ "WHERE MEMBER_ID=? ";
	...
	private static final String SELECTONE_CARTINFO="SELECT"
			+ " ADDRESS_ID, ADDRESS_NAME, ADDRESS_STREET, ADDRESS_LOTNUM, ADDRESS_DETAIL, ADDRESS_ZIPCODE "
			+ " FROM ( "
			+ "    SELECT ADDRESS_ID, ADDRESS_NAME, ADDRESS_STREET, ADDRESS_LOTNUM, ADDRESS_DETAIL, ADDRESS_ZIPCODE "
			+ "    FROM ADDRESS "
			+ "    WHERE MEMBER_ID = ? "
			+ "    ORDER BY ADDRESS_ID "
			+ " ) "
	...
	private static final String INSERT = "INSERT "
			+ "INTO ADDRESS(ADDRESS_ID, ADDRESS_NAME, ADDRESS_STREET, ADDRESS_LOTNUM, ADDRESS_DETAIL, ADDRESS_ZIPCODE, MEMBER_ID) "
			+ "VALUES((SELECT NVL(MAX(ADDRESS_ID),0)+1 FROM ADDRESS),?,?,?,?,?,?)";
	private static final String UPDATE = "UPDATE "
			+ "ADDRESS SET ADDRESS_NAME=?, ADDRESS_STREET=?, ADDRESS_LOTNUM=?, ADDRESS_DETAIL=?, ADDRESS_ZIPCODE=? "
			+ "WHERE ADDRESS_ID=?";
