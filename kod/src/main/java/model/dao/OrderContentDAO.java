...
public class OrderContentDAO {
	...
	private static final String SELECTTOP3="SELECT r.PRODUCT_ID , r.PRODUCT_IMG , r.PRODUCT_CATEGORY , r.PRODUCT_NAME , r.PRODUCT_PRICE, r.TOTAL, r.ORDERCONTENT_CNT, r.ORDERLIST_ID "
			+ "	FROM "
			+ "	(SELECT p.PRODUCT_ID , p.PRODUCT_IMG , p.PRODUCT_CATEGORY , p.PRODUCT_NAME ,p.PRODUCT_PRICE, SUM(oc.ORDERCONTENT_CNT) AS TOTAL, oc.ORDERCONTENT_CNT, oc.ORDERLIST_ID "
			+ "	FROM PRODUCT p "
			+ "	INNER JOIN ORDERCONTENT oc ON p.PRODUCT_ID = oc.PRODUCT_ID "
			+ "	GROUP BY p.PRODUCT_ID ,p.PRODUCT_IMG , p.PRODUCT_CATEGORY , p.PRODUCT_NAME ,p.PRODUCT_PRICE, oc.ORDERCONTENT_CNT, oc.ORDERLIST_ID "
