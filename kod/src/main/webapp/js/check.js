...
function check(){
  ...
  if (memberID.trim() === '') {
    ...
    // 입력한 아이디가 정규식 패턴에 맞지 않으면 메시지를 출력하고 함수 종료
	...
	// jQuery의 AJAX 함수를 호출 이함수를 사용하여 비동기적으로 서버와 통신할수있음
	$.ajax({
		type:"POST", // POST메서드를 사용하여 서버로 데이터 전송
		url : "check.do", // 요청을 보낼 서버의 주소(URL)을 지정함 
		data : {'memberID' : memberID }, // 서버로 전송할 데이터를 지정     'memberID'라는 파라미터에 memberID값을 전송
